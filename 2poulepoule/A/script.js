...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let equipes = JSON.parse(localStorage.getItem('equipesA')) || [];
    let matchs = JSON.parse(localStorage.getItem('matchsA')) || [];
    let classement = JSON.parse(localStorage.getItem('classementA')) || {};
    let historiqueMatchs = new Set(JSON.parse(localStorage.getItem('historiqueMatchsA')) || []);
    let maxTours = JSON.parse(localStorage.getItem('tours_max')) -1;
    let tours = JSON.parse(localStorage.getItem('toursA')) || [];
    let currenttour = JSON.parse(localStorage.getItem('currenttourA')) || 0;
    ...
    function initialiserClassement() {
        ...
        saveToLocalStorage('classementA', classement);
    }
    ...
    if (tours.length === 0) {
        tours = genererMatchs();
        saveToLocalStorage('toursA', tours);
    }
            ...
            for (let i = 0; i < equipesRestantes.length; i++) {
                for (let j = i + 1; j < equipesRestantes.length; j++) {
                    ...
                    if (!historiqueMatchs.has(matchID) && !equipesUtilisees.has(equipe1) && !equipesUtilisees.has(equipe2)) {
                        ...
                        saveToLocalStorage('historiqueMatchsA', Array.from(historiqueMatchs));
        ...
        if (tours[0] && tours[0].length > 0) {
            ...
            if (currenttour >= maxTours) {
                ...
                if (currenttour < maxTours) {
                    ...
                    saveToLocalStorage('currenttourA', currenttour);
    ...
    document.getElementById('resultatsForm').addEventListener('submit', function(e) {
        ...
        saveToLocalStorage('classementA', classement); // Sauvegarder le classement mis à jour
    ...
    function quelle_etape() {
        ...
        let vu = JSON.parse(localStorage.getItem('A')) || 0;
        if (etape === 2 && vu==0) {
        window.location.href = '../elim/elim.html';
            ...
            localStorage.setItem('A', JSON.stringify(vu));
            }
        }
    }
    ...
    function afficherClassement() {
        ...
        localStorage.setItem('vainqueursA', JSON.stringify(vainqueurs));
